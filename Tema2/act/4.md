...
Para obtener la información de DNS de un dominio, debemos utilizar el comando `dig` con el parámetro `-x` seguido de la dirección IP del dominio.

| Domínio | Dirección IP | Servidor DNS |
| --- | --- | --- |
| net |  |  |
| com |  |  |
| us.es |  |  |
